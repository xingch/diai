...
define(function(require, exports, module) {
	...
	/*
	require.async('tem/js/zlight/jquery.zlight.menu.1.0.min.js',function(){
		require('tem/js/zlight/zlight.menu.css');
		$(document).ready(function(){
			$('header').zlightMenu();
		})
	});
	*/
	$(document).ready(function(){
	    ...
	    $('.index_img_bg').hover(function(){
	    	$(this) .children(".bg").stop() .animate({height:"0px"},600);
	    },
	    function(){
	    ...
	    $("#index_submit").click(function(){
	    	...
	    	tel		= $("#tel").val();
	    	email	= $("#email").val();
	    	type	= $("input[name='message_type']:checked").val()
	    	message = $("#message").val();
	    	code 	= $("#code").val();
	    	if(name == '')alert('请输入姓名');return;
	    	if(tel == '')alert('请输入联系电话');return;
	    	if(email == '')alert('请输入电子邮件地址');return;
	    	...
	    	$.ajax({
	    		   type: "POST",
	    		   url: "message/message.php?action=add&from=ajax",
	    		   data: data,
	    		   success: function(data){
	    		       var srcUrl	= $("#index_code_img").attr('src');
	    		       $("#index_code_img").attr('src', srcUrl+'&'+Math.random());
	    			   alert(data );
	    		   }
	    	});
	    });
